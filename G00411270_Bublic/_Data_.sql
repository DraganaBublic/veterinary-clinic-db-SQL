...
--
-- Host: 127.0.0.1
-- Generation Time: May 09, 2023 at 09:49 PM
...
CREATE TABLE `animal` (
  ...
  `animalBreead` varchar(50) NOT NULL,
...
INSERT INTO `animal` (`animalID`, `animalName`, `animalDOB`, `animalBreead`, `animalGender`, `ownerID_fk`, `appoitmnentID_fk`) VALUES
...
(8, 'Rusty', '2008-12-29', 'Kerry Bog Pony Horse', 'Male', 1210, 3291),
...
INSERT INTO `animal_appoitment` (`ID`, `appoitmnentID_fk`, `animalID_fk`, `veterinarianID_fk`) VALUES
('', 3285, 1, 'H11_0923'),
('', 3286, 2, 'J10_0203'),
('', 3287, 3, 'H11_0923'),
('', 3288, 4, 'R18_1001'),
...
INSERT INTO `medicines_prescribed` (`medicationName`, `medicationRegime`, `medicationCost`, `examinationNo`) VALUES
('FVRCPP', 'one injection given', 17.00, 100),
('dichlorophene', '2.5 mL twice a day', 9.60, 105),
('hydroxyzine', 'given 2.5 mL for sleep', 11.50, 110),
('', 'no medication required', 0.00, 115),
('hydroxyzine', 'given 2.5 mL for sleep', 11.50, 120),
('oxymorphone', 'one tablet three times a day', 20.00, 125),
('tetanus', 'one injection given', 15.90, 130),
('EquineInfluenza', 'one injection given', 21.00, 135),
('selamectin', '3 drops per eye, twicw a day', 14.80, 140),
('IBR', 'one injection given', 9.00, 145),
('hydroxyzine, metacam', 'hydroxyzine given 1.5 mL for sleep and metacam 1 tablet per day', 42.09, 155),
('VHD -1,VHD -2', 'two injections given', 25.00, 160),
('VHD -1', 'one injection given', 15.00, 165),
('dichlorophene', '1,5 mL twice a day', 12.80, 170),
('oxymorphone', 'one tablet 2 times a day', 20.00, 175),
('metacam', '1 tablet per day for 2 weeks', 18.80, 180),
('tetanus', 'one injection given', 15.90, 185),
('metacam', '1 tablet per day for 2 weeks', 22.00, 190),
('oxytetracycline', 'every 8 hours for 2 weeks', 30.08, 195),
('', 'no medication required', 0.00, 200),
('metacam', '1 tablet per day for 2 weeks', 32.92, 205),
('null', 'one injection given', 35.00, 210);

-- --------------------------------------------------------

--
-- Stand-in structure for view `medicines_used`
-- (See below for the actual view)
--
CREATE TABLE `medicines_used` (
`medicationRegime` varchar(100)
);
...
(1002, 'William', 'Bow', '083-456852', 5, '7 Fuschia Ln, Roundstone', 'Oranmore', 'Galway', 'H91 A59Y', 'Ireland'),
...
('A11_0918', 'Amanda', 'Curran', 'Veterinary Cleaner'),
...
--
-- Structure for view `medicines_used`
--
DROP TABLE IF EXISTS `medicines_used`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `medicines_used`  AS SELECT `medicines_prescribed`.`medicationRegime` AS `medicationRegime` FROM `medicines_prescribed` WHERE `medicines_prescribed`.`medicationName` = 'metacam' ;
...
--
ALTER TABLE `animal`
  ADD CONSTRAINT `animal_ibfk_1` FOREIGN KEY (`appoitmnentID_fk`) REFERENCES `appoitment` (`appointmentID`);
